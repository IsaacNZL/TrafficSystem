...
# Queue to hold data for writing to the database
data_queue = Queue()
...
def write_to_database():
    while True:
        ...
        if data is None:  # Exit condition
            break
        # Unpack the data
        vehicle_count, date, time, car_count, bus_count, motorcycle_count = data

        # Insert data into the MariaDB table
        c.execute(
            "INSERT INTO TrafficCam1 (VehicleCount, Date, Time, Car, Bus, Motorbike) VALUES (%s, %s, %s, %s, %s, %s)",
            (vehicle_count, date, time, car_count, bus_count, motorcycle_count))
...
while True:
    ...
    # Add data to the queue for database writing
    data_queue.put((total_vehicles, date, t, car_count, bus_count, motorcycle_count))
...
# Shutdown the database writing thread
data_queue.put(None)  # Send exit signal to the database thread
db_thread.join()  # Wait for the database thread to finish
