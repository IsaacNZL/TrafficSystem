...
class DatabaseHandler:
    ...
    def insert_vehicle_data(self, table_name, total_vehicles, date, current_time, car_count, bus_count,
                            motorcycle_count):
        query = f"INSERT INTO {table_name} (VehicleCount, Date, Time, Car, Bus, Motorbike) VALUES (%s, %s, %s, %s, %s, %s)"
        self.cursor.execute(query, (total_vehicles, date, current_time, car_count, bus_count, motorcycle_count))
        self.connection.commit()
...
class ObjectDetectionModel:
    ...
    def detect_objects(self, img, thres, nms, draw=True, objects=[]):
...
class FrameProcessor(threading.Thread):
    ...
    def run(self):
        while not self.stop_flag:
            ...
            self.processed_frame_queue.put(
                (frame_with_boxes, total_vehicles, car_count, bus_count, motorcycle_count, date, current_time))
        ...
        try:
            while True:
                ...
                if not self.processed_frame_queue.empty():
                    ...
                    print(
                        f"VehicleCount: {total_vehicles} | Cars: {car_count} | Buses: {bus_count} | Motorcycles: {motorcycle_count}")

                    self.db_handler.insert_vehicle_data("TrafficCam2", total_vehicles, date, current_time, car_count,
                                                        bus_count, motorcycle_count)
...
if __name__ == "__main__":
    db_handler = DatabaseHandler(host='10.62.135.185', user='trafficcam2', password='1234', database='practicedb')
    db_handler.initialize_table("TrafficCam2")

    model = ObjectDetectionModel(
        class_file=r'/home/pi/Desktop/Object_Detection_Files/coco.names',
        config_path=r'/home/pi/Desktop/Object_Detection_Files/ssd_mobilenet_v3_large_coco_2020_01_14.pbtxt',
        weights_path=r'/home/pi/Desktop/Object_Detection_Files/frozen_inference_graph.pb'
    )
